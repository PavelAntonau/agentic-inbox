...
import { resolveMailboxBackend } from "../lib/email-helpers";
import {
  Folders,
...
export class EmailMCP extends McpAgent<Env> {
  ...
  async init() {
    ...
    /**
     * Resolve a mailbox identifier to the canonical email address used as the
     * MailboxDO key. Accepts a D1 UUID, a D1 address (case-insensitive), or
     * an R2 v1 address. Returns either the resolved `address` or an MCP error
     * if the mailbox is not registered in either stack.
     *
     * Phase 3 (2026-05-04): replaces the previous R2-only check that left
     * every tool except `list_mailboxes` unable to address D1 mailboxes.
     */
    const verifyMailbox = async (
      mailboxId: string,
    ): Promise<
      { address: string; error?: never } | ReturnType<typeof mcpError>
    > => {
      // Address path (works for both D1-by-address and R2 v1 mailboxes).
      const backend = await resolveMailboxBackend(env, mailboxId);
      if (backend) return { address: backend.row.address };
      // D1 UUID path: resolveMailboxBackend matches by lower(address) only,
      // so a UUID won't hit it. Try a direct id lookup.
      if (env.DB) {
    ...
    this.server.tool(
      ...
      async ({ mailboxId, folder, limit, page }) => {
        const __vm = await verifyMailbox(mailboxId);
        if ("isError" in __vm) return __vm;
        mailboxId = __vm.address;
