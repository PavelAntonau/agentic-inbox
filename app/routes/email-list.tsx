...
> = {
  ...
  [Folders.SENT]: {
    icon: (
      <PaperPlaneTiltIcon size={48} weight="thin" className="text-text-muted" />
    ),
  ...
  [Folders.TRASH]: {
    icon: <TrashIcon size={48} weight="thin" className="text-text-muted" />,
    ...
    <div className="animate-pulse space-y-1 p-2">
      {Array.from({ length: 8 }).map((_, i) => (
        <div key={i} className="flex items-center gap-3 px-3 py-3">
          <div className="w-4 h-4 rounded bg-card-light" />
          <div className="w-5 h-5 rounded bg-card-light" />
          <div className="flex-1 space-y-2">
            <div className="flex items-center gap-2">
              <div className="h-3 w-24 rounded bg-card-light" />
              <div className="h-3 w-4 rounded bg-card-light" />
              <div className="h-3 flex-1 rounded bg-card-light" />
              <div className="h-3 w-12 rounded bg-card-light" />
            </div>
            <div className="h-2.5 w-3/4 rounded bg-card-light" />
          </div>
        </div>
      ))}
    </div>
              ...
              return (
                ...
                >
                  ...
                  >
                    <StarIcon
                      ...
                      className={
                        email.starred
                          ? "text-kumo-warning"
                          : "text-text-muted hover:text-kumo-warning"
                      }
                    />
                  </button>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span
                        className={`truncate text-sm ${hasUnread(email) ? "font-semibold text-text-bright" : "text-text-bright"}`}
                      >
                        {formatParticipants(email)}
                      </span>
                      {(email.thread_count ?? 1) > 1 && (
                        <span className="shrink-0 text-xs text-text-muted bg-card-light rounded-full px-1.5 py-0.5 font-medium">
                          {email.thread_count}
                        </span>
                      )}
                      {email.has_draft && (
                      ...
                      <span className="text-sm text-text-muted shrink-0 ml-auto">
