...
import { Badge, Button, Loader, Pagination, Tooltip } from "~/ui";
  ...
  try {
    ...
    return parts.map((part, i) =>
      part.toLowerCase() === lowerEscaped ? (
        <mark
          key={i}
          className="bg-kumo-warning-muted text-text-bright rounded-sm px-0.5"
        >
          {part}
        </mark>
      ) : (
      ...
      <>
        <div className="flex items-center gap-2 px-4 py-3.5 border-b border-border shrink-0 md:px-5">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="text-lg font-semibold text-text-bright truncate">
              Search Results
            </h1>
            {!isLoading && (
        ...
        <div className="flex-1 overflow-y-auto">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 px-6 text-center">
              ...
              <p className="text-sm text-text-muted max-xs">
                {urlQuery
              ...
              {urlQuery && (
                <p className="text-xs text-text-muted mt-3 max-w-sm">
                  ...
                  <code className="bg-tx-card-hover px-1 rounded">
                    from:name
                  </code>
                  ,{" "}
                  <code className="bg-tx-card-hover px-1 rounded">
              ...
              {results.map((email) => {
                ...
                return (
                  <div
                    ...
                    className={`group flex items-center gap-3 w-full text-left cursor-pointer transition-colors border-b border-border px-4 py-2.5 md:px-5 md:py-3 ${isPanelOpen ? "md:px-4 md:py-2.5" : ""} ${isSelected ? "bg-tx-card-hover" : "hover:bg-tx-card-hover"}`}
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <span
                          className={`truncate text-sm ${!email.read ? "font-semibold text-text-bright" : "text-text-bright"}`}
                        >
                      ...
                      <div
                        className={`truncate text-sm mt-0.5 ${!email.read ? "font-medium text-text-bright" : "text-text-muted"}`}
                      >
