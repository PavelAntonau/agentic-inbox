...
export function useEmails(
  ...
  return useQuery<EmailListResponse>({
    ...
    // Keep previous data on key transitions and background refetches so the
    // list never flashes empty between cycles. React Query's structural
    // sharing means rows that didn't change keep referential identity, so
    // memoised row components don't re-render on every poll.
