    ...
    <div
      className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-xs font-bold ${
        isDraft
          ...
          : isSelf
            ? "bg-blue text-white"
            : "bg-card-light text-text-bright"
      }`}
    >
...
export default function ThreadMessage({
  ...
  const containerClassName = `${!isLast ? "border-b border-border" : ""} ${isDraft ? "border-l-2 border-l-yellow-400 bg-yellow-400/[0.02]" : ""}`;
    ...
    return (
      <div className={containerClassName}>
        <button
          ...
          className="w-full flex items-center gap-3 px-4 py-3 hover:bg-tx-card-hover rounded-lg text-left"
        >
          ...
          <CaretDownIcon size={14} className="text-text-muted shrink-0" />
        </button>
      </div>
    );
  }
      ...
      <div className="px-4 py-4 md:px-6">
        <div className="flex items-center justify-between gap-3 mb-3">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            >
              <div className="cursor-pointer hover:ring-2 hover:ring-blue/30 transition-shadow rounded-full">
                <Avatar
                  isDraft={isDraft}
                  isSelf={isSelf}
                  sender={email.sender}
