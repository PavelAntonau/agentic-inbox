    ...
    <>
              ...
              <table className="w-full text-sm border-collapse">
                <tbody>
                  {sourceHeaders.map((header, idx) => (
                    <tr
                      key={`${header.key}-${idx}`}
                      className={idx % 2 === 0 ? "bg-tx-card-hover/50" : ""}
                    >
                      <td className="py-1.5 px-3 font-mono font-semibold text-text-bright whitespace-nowrap align-top w-[160px]">
                        {header.key}
                      </td>
                      <td className="py-1.5 px-3 font-mono text-text-muted break-all">
                        {header.value}
                      </td>
                    </tr>
                  ))}
      ...
      >
        <Dialog size="lg">
          ...
          {previewImage && (
            <div className="mt-4 flex flex-col items-center justify-center bg-tx-card-hover/30 rounded-lg p-4 min-h-[200px]">
              <img
