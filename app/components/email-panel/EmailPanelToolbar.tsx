...
export default function EmailPanelToolbar({
  ...
  return (
    <div
      data-email-toolbar
      className="flex items-center gap-1 px-3 py-2 border-b border-border shrink-0 md:px-4"
    >
      <Button
        variant="ghost"
        shape="square"
      ...
      <div className="h-5 w-px bg-card-light mx-0.5" />
...
function MoveToFolderMenu({
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute top-full left-0 z-50 mt-1 min-w-[160px] rounded-lg border border-border bg-card shadow-lg py-1">
          ...
          {folders.map((f) => (
            <button
              key={f.id}
              type="button"
              className="w-full text-left px-3 py-1.5 text-sm text-text-bright hover:bg-tx-card-hover transition-colors"
              onClick={() => {
                onMove(f.id);
                setOpen(false);
              }}
